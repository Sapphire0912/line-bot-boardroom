...
export async function POST(req: Request) {
  try {
    ...
    if (!username || !account || !newPassword)
      return NextResponse.json({ message: "必填區域不可為空", status: 400 });

    // 檢查使用者是否存在
    const isExistUser = await User.findOne({ username });
    if (!isExistUser)
      return NextResponse.json({ message: "該使用者不存在", status: 400 });
    ...
    return NextResponse.json({ message: "伺服器出現錯誤", status: 500 });
  }
}
